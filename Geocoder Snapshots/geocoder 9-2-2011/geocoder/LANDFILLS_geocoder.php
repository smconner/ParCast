...
function geocode_table( $oMySQL ) {
	...
	$GEO_MYSQL_TABLE_NAME = "LANDFILLS";
	
	// identify the column names for the table define by: $GEO_MYSQL_TABLE_NAME
	$GEO_ADDRESS 		= "LOCATION_ADDRESS";
	$GEO_ADDRESS_CITY 	= "CITY_NAME";
	$GEO_ADDRESS_STATE 	= "STATE_CODE" ;
	$GEO_ADDRESS_ZIP 	= "POSTAL_CODE" ;
	$GEO_LAT_COLUMN		= "Latitude2"; // what to name the lat column
	$GEO_LON_COLUMN		= "Longitude2"; // what to name the lon column
	 ...
	 */
		...
		geo_yahoo( $oMySQL, $GEO_MYSQL_TABLE_NAME, $GEO_ADDRESS, $GEO_ADDRESS_CITY, $GEO_ADDRESS_STATE, $GEO_ADDRESS_ZIP, $GEO_LON_COLUMN, $GEO_LAT_COLUMN, "YAHOO_FULL_ADDRESS" );
	
	
	/**
	 *	'Last resort' attempts to geolocate down to at least the city level NOTE: do not use if Google API starts rejecting requests
	 */
...
function geo_yahoo( $oMySQL, $GEO_MYSQL_TABLE_NAME, $GEO_ADDRESS, $GEO_ADDRESS_CITY, $GEO_ADDRESS_STATE, $GEO_ADDRESS_ZIP, $GEO_LON_COLUMN, $GEO_LAT_COLUMN, $GEOCODE_METHOD ) {
	...
	foreach ( $MYSQL_ALL_ROWS_ARRAY as $row_number_key => $row_array_value ) {
		...
		$YAHOO_API_OPTIONS_STRING = "&flags=J" . "&flags=C" . "&count=1" . "&appid=JKpwM84k";
			...
			$YAHOO_API_ADDRESS_STRING = " $row_array_value[$GEO_ADDRESS], $row_array_value[$GEO_ADDRESS_CITY], $row_array_value[$GEO_ADDRESS_STATE] $row_array_value[$GEO_ADDRESS_ZIP]";
