...
function RCRA_bigtable( $oMySQL ) {
	...
	// pull all the records from the tables accessed by HANDLER_ID
	//populate_bigtable_by_HANDLER_ID($oMySQL, $TABLE_NAME, $TABLE_INDEX, $API_QUERY_STRING . "/RCR_PUNIT_DETAIL/HANDLER_ID/" , $ROWS);
...
function populate_bigtable_by_HANDLER_ID( $oMySQL, $TABLE_NAME, $TABLE_INDEX, $API_QUERY_STRING, $ROWS ) {
	
	error_log("Populating table: $TABLE_NAME by HANDLER_ID.");
	
	// maybe use: serialize($value)
	
	// http://iaspub.epa.gov/enviro/efservice/RCR_PUNIT_DETAIL/HANDLER_ID/OHD980683544/
	// add gorss number of PA (punitive actions)
