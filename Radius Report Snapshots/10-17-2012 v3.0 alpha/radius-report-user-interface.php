...
function radius_report_dashboard_widget_function() {
    ...
    $current_user_ID = $current_user->ID;
		...
		if ( file_exists($user_directory_path) ){
			...
			foreach ( $file_list_pdf as $key => $value ) {
				...
				if ( in_array( $basename." supplements.zip", $file_list_zip) ) {
					print("<a href=\"". $user_directory_link_path ."/". $basename." supplements.zip\">[Download Supplements]</a>\n");
				}
	...
	$message = '<br><br>
				<b> We are trying something new:</b> Download the <b>"Supplements"</b> file for valuable cross-reference information about your search.
				<br><br>';
	echo $message;
