...
function geocode_table( $oMySQL ) {
	...
	/**
	 *	Adds the new Latitude and Longitude columns
	 */
	 ...
	 *	NOTE: Be sure that zip codes only have 5 digits - both OSM and GOOGLE will reject many requests that don't have 5 digit zip codes.
	 */
...
/**
 * TODO: create two functions, one that adds a permenent column called GEOCODE_METHOD, 
 *  and another that takes a number, which indicates the following:
 * 0 = not encoded
 * 1 = encoded by OSM API with "last resort"
 * 2 = encoded by Google API with "last resort"
 * 3 = encoded by OSM API with full address
 * 4 = encoded by Google API with full address
 *  
 */
...
/**
 * Simply adds a column ofzeros the table being geocoded called GEOCODE_METHOD
 * @param object $oMySQL
...
function geo_add_geocode_method( $oMySQL, $GEO_MYSQL_TABLE_NAME ) {
	...
	$sql = "ALTER TABLE `$GEO_MYSQL_TABLE_NAME` ADD `GEOCODE_METHOD` INT(2) NOT NULL";
...
function geo_osm( $oMySQL, $GEO_MYSQL_TABLE_NAME, $GEO_ADDRESS, $GEO_ADDRESS_CITY, $GEO_ADDRESS_STATE, $GEO_ADDRESS_ZIP, $GEO_LON_COLUMN, $GEO_LAT_COLUMN ) {
	...
	// grab all the addresses and existing lat lon columns
		...
		if ( $row_array_value[$GEO_LAT_COLUMN] == 0 || $row_array_value[$GEO_LON_COLUMN] == 0 || $row_array_value[$GEO_LAT_COLUMN] == NULL || $row_array_value[$GEO_LON_COLUMN] == NULL || !is_numeric( $row_array_value[$GEO_LAT_COLUMN] ) || !is_numeric( $row_array_value[$GEO_LON_COLUMN] ) ) {
				...
				// if OSM query address IS the same as the last, insert the last geolocation into the current row 
					...
					if ( $OSM_API_QUERY_RESULTS != "[]" ) {
						...
						$OSM_GEO_LAT_LON = "UPDATE `$GEO_MYSQL_TABLE_NAME` SET `$GEO_LAT_COLUMN` = $GEO_LAT, `$GEO_LON_COLUMN` = $GEO_LON WHERE GEO_ID = $GEO_ID";
		...
		if ( $row_array_value[$GEO_LAT_COLUMN] == 0 || $row_array_value[$GEO_LON_COLUMN] == 0 || $row_array_value[$GEO_LAT_COLUMN] == NULL || $row_array_value[$GEO_LON_COLUMN] == NULL || !is_numeric( $row_array_value[$GEO_LAT_COLUMN] ) || !is_numeric( $row_array_value[$GEO_LON_COLUMN] ) ) {
				...
				// if GOOGLE query address IS the same as the last, insert the last geolocation into the current row 
					...
					if ( $GOOGLE_API_QUERY_RESULTS_ARRAY["status"] == "OK" ) {
						...
						$GOOGLE_GEO_LAT_LON = "UPDATE `$GEO_MYSQL_TABLE_NAME` SET `$GEO_LAT_COLUMN` = $GEO_LAT, `$GEO_LON_COLUMN` = $GEO_LON WHERE GEO_ID = $GEO_ID";
