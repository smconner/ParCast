...
function groupBigArray( $bigArray ) {
	...
	/**
	 *	[DB_NAME] => CERCLISNFRAP via the [SITE_ID] => WAD009261397
	 */
	...
	/**
	 *	[DB_NAME] => CERCLIS via the [SITE_ID] => WAD009264284
	 */
 ...
 * @abstract <p> 		1) Given the distance 'dist' in miles or kilometers. <br />
 *						2) Let radiusEarth = 6372.7976 km or radiusEarth=3959.8728 miles <br />
 *						3) Convert distance to the distance in radians. <br />
 *								dist = dist/radiusEarth <br />
 *				    	4) Calculate the destination coordinates. <br />
 *							    lat2 = asin(sin(lat1)*cos(dist) + cos(lat1)*sin(dist)*cos(brg)) <br />
 *							    lon2 = lon1 + atan2(sin(brg)*sin(dist)*cos(lat1), cos(dist)-sin(lat1)*sin(lat2)) <br />
 *						5) Calculate the final bearing and back bearing. <br />
 *							    dLon = lon1-lon2 <br />
 *							    y = sin(dLon) * cos(lat1) <br />
 *							    x = cos(lat2)*sin(lat1) - sin(lat2)*cos(lat1)*cos(dLon) <br />
 *							    d=atan2(y, x) <br />
 *							    finalBrg = d+PI <br />
 *							    backBrg=d+2*PI <br />
 *						6) Convert lat2, lon2, finalBrg and backBrg to degrees <br />
 *							    lat2 = lat2 * 180/PI <br />
 *							    lon2 = lon2 * 180/PI <br />
 *							    finalBrg = finalBrg * 180/PI <br />
 *							    backBrg = backBrg * 180/PI <br />
 *						7) If lon2 is outside the range -180 to 180, add or subtract 360 to bring it back into that range. <br />
 *						8) If finalBrg or backBrg is outside the range 0 to 360, add or subtract 360 to bring them back into that range. </p>
**/
...
function groupDataByKey( $bigArray, $DB_NAME_ARRAY, $DB_KEY ) {
		...
		if ( !empty($bigArray["DATA"]) ) {
				...
				foreach ( $bigArray["DATA"] as $key => $this_array ) {
					...
					// if this current sub-array has the database name CERCLISNFRAP
