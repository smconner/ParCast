...
class MySQL {
	...
	function MySQL(){
		//$this->Connect($sHostname = false, $sUsername = false, $sPassword = false, $sDatabase = false);
	}
	
	// Connects class to database
	// $bPersistant (boolean) - Use persistant connection?
	function Connect( $sHostname, $sUsername, $sPassword, $sDatabase, $bPersistant = false ){
		if($this->sDBLink){
			mysql_close($this->sDBLink);
		}
		
		if($bPersistant){
			$this->sDBLink = mysql_pconnect($sHostname, $sUsername, $sPassword);
		}else{
			$this->sDBLink = mysql_connect($sHostname, $sUsername, $sPassword);
		}
