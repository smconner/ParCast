...
// create a function that can build and update a big table from the EPA API RCRA data tables
...
function RCRA_bigtable( $oMySQL ) {
	...
	make_prototype_table( $oMySQL, $TABLE_NAME, $TABLE_INDEX, $API_ROOT_STRING . "/RCR_CA_AUTHORITY/HANDLER_ID/WAD058362336/rows/1:1" );
	make_prototype_table( $oMySQL, $TABLE_NAME, $TABLE_INDEX, $API_ROOT_STRING . "/RCR_PSERIES/HANDLER_ID/WAD058362336/rows/1:1" );

	
	// pull all the records from: RCR_HREPORT_UNIV and populate the table: RCRAINFO for WA, OR, and CA
	populate_bigtable( $oMySQL, $TABLE_NAME, $TABLE_INDEX, $API_ROOT_STRING . "/RCR_HREPORT_UNIV/LOCATION_STATE/", $ROWS, $STATE );

	// use the index: HANDLER_ID from the table: RCRAINFO and pull all the records and populate the table: RCRAINFO
...
function populate_bigtable( $oMySQL, $TABLE_NAME, $TABLE_INDEX, $API_QUERY_STRING, $ROWS, $STATE ) {
			...
			foreach ( $API_QUERY_RESULTS_ARRAY as $master_key => $master_value ) {
				...
				foreach ( $master_value as $child_key => $child_value ) { // here we have access to the array elements to make column names
					...
					if ( $child_key != "Count" && $child_key != "Rows" ) { // bypass the "Count" and "Rows" keys
						...
						$sql_string = "INSERT INTO " .$TABLE_NAME. " (" . implode(', ', $sql_col) . ") VALUES (" . implode(', ', $sql_row) . ")";
...
function make_prototype_table( $oMySQL, $TABLE_NAME, $TABLE_INDEX, $API_QUERY_STRING ) {
		...
		foreach ( $API_QUERY_RESULTS_ARRAY as $master_key => $master_value ) {
			...
			foreach ( $master_value as $child_key => $child_value ) { // here we have access to the array elements to make column names
				...
				if ( $child_key != "Count" && $child_key != "Rows" ) { // bypass the "Count" and "Rows" keys
					...
					// test for columns
					if ( !mysql_num_rows( mysql_query( "SHOW COLUMNS FROM $TABLE_NAME LIKE '".$child_key."'" ) ) ) {
						
						//error_log("Column $child_key does not exist, creating it");
						
						$oMySQL->ExecuteSQL( "ALTER TABLE $TABLE_NAME ADD COLUMN ($child_key TEXT NOT NULL)" );
												

					} else {
						
						//error_log("Column $child_key already exists");
