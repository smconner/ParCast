...
// 1. get a worst case bounding-box for the state / zip
// 2. check that the first case is within the worst case bounding box
// 3. if it is NOT then use the geo_last_resort_google function
...
define( "GEO_MYSQL_DB_NAME", "UST_WA_DB" );
...
function geo_add_geocode_method( $oMySQL, $GEO_MYSQL_TABLE_NAME ) {
	...
	$sql_one = "ALTER TABLE `$GEO_MYSQL_TABLE_NAME` ADD `GEOCODE_METHOD` VARCHAR(30) NOT NULL";
	...
	foreach ( $MYSQL_ALL_ROWS_ARRAY as $row_number_key => $row_array_value ) {
		...
		$YAHOO_API_OPTIONS_STRING = "&flags=J" . "&flags=C" . "&count=1" . "&appid=JKpwM84k";
			...
			$YAHOO_API_ADDRESS_STRING = "$row_array_value[$GEO_ADDRESS], $row_array_value[$GEO_ADDRESS_CITY], $row_array_value[$GEO_ADDRESS_STATE] $row_array_value[$GEO_ADDRESS_ZIP]";
				...
				$YAHOO_API_ADDRESS_STRING = preg_replace( "/\&+/", "AND", $YAHOO_API_ADDRESS_STRING );	# replace & symbol with AND
